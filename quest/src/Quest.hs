...
compress []=[]
compress (x:[])=[x]
compress (x:x1:xs)|x==x1=compress (x:xs)
...
encodeM []=[]
encodeM  (x:xs)=ecode xs  ((One x):[]) where
...
encodeMM =map helper.encode where
                                            helper(x,1)=One x
                                            helper(x,n)=Multi x n
decodeM ::[ItemE a]->[a]
decodeM =foldr (\x->((h x)++))[] where
                                    h (One a)=[a]
                                    h (Multi a n)=replicate n a
decodeMM ::[ItemE a]->[a]
decodeMM =concatMap h  where
                                    h (One a)=[a]
                                    h (Multi a n)=replicate n a
dupl::[a]->[a]
...
remo1::Int->[a]->(a,[a])
remo1 nn xx= re nn xx (Nothing,[]) where
...
sortt::(Ord a)=>[a]->[a]
sortt []=[]
sortt (x:xs:xss)|x>xs=x:sortt(xs:xss)
                |x<=xs=xs:sortt(x:xss)
sortt (x:[])=[x]
